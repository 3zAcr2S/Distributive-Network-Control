...
void formating() {
  ...
  d1        = sqrt(sq(Z1[0]) + sq(Z1[1]) + 0 * sq(Z1[2]));
  d2        = sqrt(sq(Z2[0]) + sq(Z2[1]) + 0 * sq(Z2[2]));



  velocity  = round(k_d * ( (d1 - d_ideal) + (d2 - d_ideal) ) );
  dist = round(-0.0067314 * pow(velocity, 2) + 5.024336 * velocity - 5.107447);
...
void getdistance() {
  ...
  angle = (atan(Z1[1] / Z1[0]) + atan(Z2[1] / Z2[0])) / 2;
}
