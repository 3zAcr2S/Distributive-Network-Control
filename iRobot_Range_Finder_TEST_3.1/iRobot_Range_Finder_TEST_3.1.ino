...
#define k_d             1
#define rotation_speed   3.49065850399     // = 2 pi / 1.8 [sec]
#define d_ideal         120               // [cm]
...
void setup()
{
  ...
  Serial1.begin(19200); //iRobot

  delay(2000); // NEEDED!!!! To let the robot initialize
  Serial1.write(128); // This command starts the OI. You must always send the Start command before sending any other commands to the OI
  delay(50);
  Serial1.write(131); // Go into save mode, i.e. whenever robot is picked up it stops (see p. 6 of Create's Open Interface documentation)
  delay(50);
...
void formating() {
  ...
  velocity = sqrt(sq(V1) + sq(V2)) / (pow(10, 5));
  Serial.println(velocity);
  angle = atan2(V2, V1);
  Serial.println(angle);
  //  radius    = 1;
  //
  //  t_r = angle / rotation_speed * 1000; // in millisecond;
  //  t_rotate = millis();
  //  while ( (millis() - t_rotate) < t_r ){
  //      drive(500, radius);
  //  }
  //  delay(1000);
  //  t_move = millis();
  //  while ( (millis() - t_move) < 2000 ){
  //    driveStraight(V1);
  //  }
  //
  ...
  t_r = round((-0.0545 * pow(t0, 2) + 1.151 * t0 - 0.1161) * 1000);
  //Serial.print(t_r);
  //Serial.print("  ");
  //Serial.println(t_form);

  if (t_r > 0) {
    while ( (millis() - t_form) < t_r)
    {
      drive(500, 1);
      //Serial.println("Should drive");
    }
  }
  else {
    while ( (millis() - t_form) < (-t_r) )
  ...
  {
    driveStraight(-0.0067 * pow(velocity, 2) + 5.0243 * velocity - 5.1074);
  }
